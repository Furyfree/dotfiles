...
all state. Nimbus also provides an explicit, approved lockscreen-widget repair
with a private backup and shell restart; see [README.md](README.md#login-and-lockscreen-appearance).
A normal Chezmoi apply does not erase GUI experiments. Selecting both
...
| Idle and locking | Native idle actions lock after five minutes, then turn screens off after one idle minute locked (`locked_timeout = 60`). A five-second cancellable dim precedes each action. The normal screen-off timeout is six minutes; locking re-arms the timers. Inhibitors are respected. No automatic suspend or second idle daemon is configured. |
| Night light | Native `[nightlight] enabled = true` follows the `[location]` auto-locate schedule. Force and Kelvin stay at upstream defaults. |
| Icon discovery | Chezmoi extends the installed hicolor index and refreshes the user cache for local app icons, including package links for T3 Code and Yazi. Papirus copies and folder-color links remain generated by the existing native theme hook. |
...
the native actions, with Sleep using `lock_and_suspend` so locking completes
before suspend. Logout, Reboot and Shutdown have five-second cancellable
countdowns; Lock and Sleep have none. These countdowns do not provide app-save
prompts. GUI row overrides still take precedence.
